...
#include <stdlib.h>
  ...
  static inline OnnxModelInfo onnx_model_get_info(OnnxModel* model)
  {
    ...
    status = model->api->SessionGetInputName(
      model->session, 0, model->allocator, &input_names);
    // printf("[DEBUG] load shit : %s\n", input_names);
    onnx_model_check_status(model->api, status);

    status =
      model->api->SessionGetOutputCount(model->session, &num_output_nodes);
    onnx_model_check_status(model->api, status);
  ...
  static inline OnnxModel* onnx_model_load(const char* model_path)
  {
    ...
    OrtStatus* status = model->api->CreateEnv(
      ORT_LOGGING_LEVEL_WARNING, "onnx-model", &model->env);
    if (status != NULL) {
      onnx_model_check_status(model->api, status);
      free(model);
      return NULL;
