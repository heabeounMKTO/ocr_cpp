...
PADDLE_API Tensor accuracy_check(const Tensor& x, const Tensor& y, const std::string& fn_name, double rtol = 1e-5, double atol = 1e-8, bool equal_nan = false);
...
PADDLE_API Tensor add_position_encoding(const Tensor& x, float alpha = 1.0f, float beta = 1.0f);
...
PADDLE_API Tensor affine_channel(const Tensor& x, const Tensor& scale, const Tensor& bias, const std::string& data_layout = "AnyLayout");
...
PADDLE_API Tensor all_gather(const Tensor& x, int ring_id = 0, int nranks = 0);

PADDLE_API Tensor all_reduce(const Tensor& x, int ring_id = 0, int reduce_type = 0);

PADDLE_API Tensor& all_reduce_(Tensor& x, int ring_id = 0, int reduce_type = 0);

PADDLE_API Tensor all_to_all(const Tensor& x, int ring_id = 0);
...
PADDLE_API std::tuple<Tensor, Tensor> argsort(const Tensor& x, int axis = -1, bool descending = false, bool stable = false);
...
PADDLE_API Tensor baddbmm(const Tensor& input, const Tensor& x, const Tensor& y, float beta = 1.0, float alpha = 1.0);

PADDLE_API Tensor& baddbmm_(Tensor& input, const Tensor& x, const Tensor& y, float beta = 1.0, float alpha = 1.0);

PADDLE_API Tensor barrier(const Tensor& x, int ring_id = 0);
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor> beam_search(const Tensor& pre_ids, const Tensor& pre_scores, const paddle::optional<Tensor>& ids, const Tensor& scores, int level, int beam_size, int end_id, bool is_accumulated = true);
...
PADDLE_API std::tuple<Tensor, Tensor> bipartite_match(const Tensor& dist_mat, const std::string& match_type = "bipartite", float dist_threshold = 0.5);
...
PADDLE_API Tensor broadcast(const Tensor& x, int ring_id = 0, int root = 0);
...
PADDLE_API Tensor c_scatter(const Tensor& x, int ring_id = 0, int root = 0, int nranks = 0, bool use_calc_stream = false);

PADDLE_API std::tuple<Tensor, Tensor> c_softmax_with_cross_entropy(const Tensor& logits, const Tensor& label, int64_t ignore_index = -100, int ring_id = 0, int rank = 0, int nranks = 0);

PADDLE_API Tensor c_split(const Tensor& x, int rank = 0, int nranks = 1, int ring_id = 0, bool use_model_parallel = true);
...
PADDLE_API Tensor correlation(const Tensor& input1, const Tensor& input2, int pad_size, int kernel_size, int max_displacement, int stride1, int stride2, int corr_type_multiply = 1);
...
PADDLE_API std::tuple<Tensor, Tensor> ctc_align(const Tensor& input, const paddle::optional<Tensor>& input_length, int blank = 0, bool merge_repeated = true, int padding_value = 0);
...
PADDLE_API Tensor cumprod(const Tensor& x, int dim, bool exclusive = false, bool reverse = false);
...
PADDLE_API Tensor cvm(const Tensor& x, const Tensor& cvm, bool use_cvm = true);

PADDLE_API Tensor data(const std::string& name, const IntArray& shape, DataType dtype, const Place& place);

PADDLE_API std::tuple<Tensor, Tensor> decayed_adagrad(const Tensor& param, const Tensor& grad, const Tensor& moment, const Tensor& learning_rate, float decay = 0.95f, float epsilon = 1.0e-6f);
...
PADDLE_API Tensor dgc_clip_by_norm(const Tensor& x, const Tensor& current_step, float max_norm, float rampup_begin_step = -1.0);
...
PADDLE_API Tensor dpsgd(const Tensor& param, const Tensor& grad, const Tensor& learning_rate, float clip = 10.0f, float batch_size = 16.0f, float sigma = 1.0f, int seed = 0);
...
PADDLE_API Tensor embedding_with_scaled_gradient(const Tensor& x, const Tensor& weight, int64_t padding_idx = -1);
...
PADDLE_API Tensor fake_channel_wise_dequantize_max_abs(const Tensor& x, const std::vector<Tensor>& scales, const std::vector<int>& quant_bits = {8}, int quant_axis = 0, int x_num_col_dims = 1);

PADDLE_API std::tuple<Tensor, Tensor> fake_channel_wise_quantize_abs_max(const Tensor& x, int bit_length = 8, int round_type = 1, int quant_axis = 0, bool is_test = false);

PADDLE_API std::tuple<Tensor, Tensor> fake_channel_wise_quantize_dequantize_abs_max(const Tensor& x, int bit_length = 8, int round_type = 1, int quant_axis = 0);

PADDLE_API Tensor fake_dequantize_max_abs(const Tensor& x, const Tensor& scale, float max_range);

PADDLE_API std::tuple<Tensor, Tensor> fake_quantize_abs_max(const Tensor& x, int bit_length = 8, int round_type = 1);

PADDLE_API std::tuple<Tensor, Tensor> fake_quantize_dequantize_abs_max(const Tensor& x, int bit_length = 8, int round_type = 1);

PADDLE_API std::tuple<Tensor, Tensor, Tensor, Tensor> fake_quantize_dequantize_moving_average_abs_max(const Tensor& x, const Tensor& in_scale, const paddle::optional<Tensor>& in_accum, const paddle::optional<Tensor>& in_state, float moving_rate = 0.9, int bit_length = 8, bool is_test = false, int round_type = 1);
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor, Tensor> fake_quantize_moving_average_abs_max(const Tensor& x, const Tensor& in_scale, const paddle::optional<Tensor>& in_accum, const paddle::optional<Tensor>& in_state, float moving_rate = 0.9, int bit_length = 8, bool is_test = false, int round_type = 1);
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor> fake_quantize_range_abs_max(const Tensor& x, const Tensor& in_scale, const paddle::optional<Tensor>& iter, int window_size = 10000, int bit_length = 8, bool is_test = false, int round_type = 1);
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor, Tensor> flash_attn_qkvpacked(const Tensor& qkv, const paddle::optional<Tensor>& fixed_seed_offset, const paddle::optional<Tensor>& attn_mask, float dropout = 0.0, bool causal = false, bool return_softmax = false, bool is_test = false, const std::string& rng_name = "");
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor> ftrl(const Tensor& param, const Tensor& squared_accumulator, const Tensor& linear_accumulator, const Tensor& grad, const Tensor& learning_rate, float l1 = 0.0f, float l2 = 0.0f, float lr_power = -0.5f);
...
PADDLE_API Tensor global_gather(const Tensor& x, const Tensor& local_count, const Tensor& global_count, int ring_id = 0);

PADDLE_API Tensor global_scatter(const Tensor& x, const Tensor& local_count, const Tensor& global_count, int ring_id = 0);
...
PADDLE_API Tensor gru(const Tensor& input, const paddle::optional<Tensor>& h0, const Tensor& weight, const paddle::optional<Tensor>& bias, const std::string& activation = "tanh", const std::string& gate_activation = "sigmoid", bool is_reverse = false, bool origin_mode = false, bool is_test = false);

PADDLE_API Tensor gru_unit(const Tensor& input, const Tensor& hidden_prev, const Tensor& weight, const paddle::optional<Tensor>& bias, int activation = 2, int gate_activation = 1, bool origin_mode = false);
...
PADDLE_API Tensor histogram(const Tensor& input, const paddle::optional<Tensor>& weight, int64_t bins = 100, float min = 0.0, float max = 0.0, bool density = false);
...
PADDLE_API Tensor im2sequence(const Tensor& x, const paddle::optional<Tensor>& y, const std::vector<int>& kernels, const std::vector<int>& strides = {1, 1}, const std::vector<int>& paddings = {0, 0, 0, 0}, const std::vector<int>& out_stride = {1, 1});
...
PADDLE_API Tensor kldiv_loss(const Tensor& x, const Tensor& label, const std::string& reduction = "mean", bool log_target = false);
...
PADDLE_API Tensor logsumexp(const Tensor& x, const std::vector<int>& axis = {0}, bool keepdim = false, bool reduce_all = false);

PADDLE_API Tensor lookup_table_dequant(const Tensor& w, const Tensor& ids, int64_t padding_idx = -1);

PADDLE_API Tensor lp_pool2d(const Tensor& x, const IntArray& kernel_size, const std::vector<int>& strides = {1,1}, const std::vector<int>& paddings = {0,0}, bool ceil_mode = false, bool exclusive = true, const std::string& data_format = "NCHW", const std::string& pooling_type = "", bool global_pooling = false, bool adaptive = false, const std::string& padding_algorithm = "EXPLICIT", float norm_type = 0.0f);
...
PADDLE_API Tensor lu_solve(const Tensor& b, const Tensor& lu, const Tensor& pivots, const std::string& trans);
...
PADDLE_API std::tuple<Tensor, Tensor> match_matrix_tensor(const Tensor& x, const Tensor& y, const Tensor& w, int dim_t = 1);
...
PADDLE_API Tensor matrix_rank_atol_rtol(const Tensor& x, const Tensor& atol, const paddle::optional<Tensor>& rtol, bool hermitian = false);
...
PADDLE_API std::tuple<Tensor, Tensor> max_pool2d_with_index(const Tensor& x, const std::vector<int>& kernel_size, const std::vector<int>& strides = {1, 1}, const std::vector<int>& paddings = {0, 0}, bool global_pooling = false, bool adaptive = false, bool ceil_mode = false);

PADDLE_API std::tuple<Tensor, Tensor> max_pool3d_with_index(const Tensor& x, const std::vector<int>& kernel_size, const std::vector<int>& strides = {1, 1, 1}, const std::vector<int>& paddings = {0, 0, 0}, bool global_pooling = false, bool adaptive = false, bool ceil_mode = false);
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> moe_dispatch(const Tensor& X, const Tensor& gating_output, int moe_topk = 2, bool group_moe = false, bool topk_only_mode = false);

PADDLE_API Tensor moe_ffn(const Tensor& permute_input, const Tensor& token_nums_per_expert, const Tensor& ffn1_weight, const Tensor& ffn2_weight, const paddle::optional<Tensor>& ffn1_bias, const paddle::optional<Tensor>& ffn1_scale, const paddle::optional<Tensor>& ffn2_scale, const std::string& quant_method);

PADDLE_API Tensor moe_reduce(const Tensor& ffn_out, const Tensor& expert_scales_float, const Tensor& permute_indices_per_token, const Tensor& top_k_indices, const paddle::optional<Tensor>& ffn2_bias, bool norm_topk_prob = false, float routed_scaling_factor = 1.0);
...
PADDLE_API Tensor mp_allreduce_sum(const Tensor& x, int ring_id = 0);
...
PADDLE_API Tensor partial_allgather(const Tensor& x, int nranks, int rank, int ring_id = 0);

PADDLE_API Tensor& partial_allgather_(Tensor& x, int nranks, int rank, int ring_id = 0);

PADDLE_API Tensor partial_concat(const std::vector<Tensor>& x, int start_index = 0, int length = -1);

PADDLE_API Tensor partial_sum(const std::vector<Tensor>& x, int start_index = 0, int length = -1);
...
PADDLE_API Tensor prune_gate_by_capacity(const Tensor& gate_idx, const Tensor& expert_count, int64_t n_expert = 0, int64_t n_worker = 0);
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor> rank_attention(const Tensor& x, const Tensor& rank_offset, const Tensor& rank_param, int max_rank = 3, int max_size = 0);
...
PADDLE_API Tensor reduce(const Tensor& x, int ring_id = 0, int root_id = 0, int reduce_type = 0);
...
PADDLE_API Tensor reduce_scatter(const Tensor& x, int ring_id = 0, int nranks = 1);
...
PADDLE_API Tensor restrict_nonzero(const Tensor& condition, int64_t total_true_num);
...
PADDLE_API Tensor round(const Tensor& x, int decimals = 0);
...
PADDLE_API Tensor rrelu(const Tensor& x, float lower = 1.0f/8, float upper = 1.0f/3, bool is_test = false);
...
PADDLE_API Tensor sequence_conv(const Tensor& x, const paddle::optional<Tensor>& padding_data, const Tensor& filter, int context_length, bool padding_trainable = false, int context_start = 0, int context_stride = 1);

PADDLE_API Tensor sequence_mask(const Tensor& x, const Scalar& max_len, DataType out_dtype);

PADDLE_API Tensor sequence_pool(const Tensor& x, bool is_test = false, const std::string& pooltype = "AVERAGE", float pad_value = 0.0);

PADDLE_API Tensor set(const Tensor& x, const Tensor& source, const std::vector<int64_t>& dims = {}, const std::vector<int64_t>& stride = {}, int64_t offset = 0);
...
PADDLE_API Tensor shape64(const Tensor& input);
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor> shuffle_batch(const Tensor& x, const Tensor& seed, int startup_seed = 0);

PADDLE_API Tensor shuffle_channel(const Tensor& x, int group = 1);
...
PADDLE_API Tensor svdvals(const Tensor& x);
...
PADDLE_API std::tuple<Tensor, Tensor> tdm_child(const Tensor& x, const Tensor& tree_info, int child_nums, DataType dtype = DataType::INT32);

PADDLE_API std::tuple<Tensor, Tensor, Tensor> tdm_sampler(const Tensor& x, const Tensor& travel, const Tensor& layer, bool output_positive = true, const std::vector<int>& neg_samples_num_list = {}, const std::vector<int>& layer_offset = {}, int seed = 0, int dtype = 2);

PADDLE_API Tensor temporal_shift(const Tensor& x, int seg_num, float shift_ratio = 0.25f, const std::string& data_format = "NCHW");

PADDLE_API Tensor thresholded_relu(const Tensor& x, float threshold = 1.0, float value = 0.0);

PADDLE_API Tensor& thresholded_relu_(Tensor& x, float threshold = 1.0, float value = 0.0);

PADDLE_API std::tuple<Tensor, Tensor, Tensor, Tensor> top_p_sampling(const Tensor& x, const Tensor& ps, const paddle::optional<Tensor>& threshold, const paddle::optional<Tensor>& topp_seed, int seed = -1, int k = 0, const std::string& mode = "truncate");
...
PADDLE_API Tensor truncated_gaussian_random(const std::vector<int>& shape, float mean, float std, int seed, float a, float b, DataType dtype = DataType::FLOAT32, const Place& place = {});
...
PADDLE_API Tensor uniform_random_batch_size_like(const Tensor& input, const std::vector<int>& shape, int input_dim_idx = 0, int output_dim_idx = 0, float min = -1.0f, float max = 1.0f, int seed = 0, int diag_num = 0, int diag_step = 0, float diag_val = 1.0f, DataType dtype = DataType::FLOAT32);
...
PADDLE_API Tensor variance(const Tensor& x, const std::vector<int64_t>& axis = {}, bool keepdim = false);

PADDLE_API Tensor view_dtype(const Tensor& input, DataType dtype);

PADDLE_API Tensor view_slice(const Tensor& input, int64_t begin_idx, int64_t end_idx);
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor, Tensor> chunk_eval(const Tensor& inference, const Tensor& label, const paddle::optional<Tensor>& seq_length, int num_chunk_types, const std::string& chunk_scheme = "IOB", const std::vector<int>& excluded_chunk_types = {});
