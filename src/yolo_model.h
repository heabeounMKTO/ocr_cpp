...
cv::Mat YOLO12Detector::preprocess(const cv::Mat &image, float *&blob,
                                   std::vector<int64_t> &inputTensorShape) {
  ...
  utils::letterBox(image, resizedImage, inputImageShape,
                   cv::Scalar(114, 114, 114), isDynamicInputShape, false, true,
                   32);
...
std::vector<Detection> YOLO12Detector::detect(const cv::Mat &image,
                                              float confThreshold,
                                              float iouThreshold) {
