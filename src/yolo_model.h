 ...
 */
static inline void
letterBox (const cv::Mat &image, cv::Mat &outImage, const cv::Size &newShape,
...
scaleCoords (const cv::Size &imageShape, BoundingBox coords,
  ...
  if (p_Clip)
    {
      ...
      result.width = utils::clamp (result.width, 0,
                                   imageOriginalShape.width - result.x);
      result.height = utils::clamp (result.height, 0,
      ...
      for (size_t j = i + 1; j < sortedIndices.size (); ++j)
        {
          ...
          const float x1
              = std::max (x1_max, static_cast<float> (compareBox.x));
          const float y1
              = std::max (y1_max, static_cast<float> (compareBox.y));
...
generateColors (const std::vector<std::string> &classNames, int seed = 42)
{
  ...
  std::mt19937 rng (
      seed); // Initialize random number generator with fixed seed
  std::uniform_int_distribution<int> uni (
      0, 255); // Define distribution for color values

  for (size_t i = 0; i < classNames.size (); ++i)
    {
      colors.emplace_back (cv::Scalar (
          uni (rng), uni (rng), uni (rng))); // Generate random BGR color
    }
 ...
 */
static inline void
drawBoundingBox (cv::Mat &image, const std::vector<Detection> &detections,
 ...
 */
static inline void
drawBoundingBoxMask (cv::Mat &image, const std::vector<Detection> &detections,
                     const std::vector<std::string> &classNames,
                     ...
                     float maskAlpha = 0.4f)
{
  ...
  const int textThickness = std::max (
      1, static_cast<int> (std::min (imgHeight, imgWidth) * 0.001));
...
public:
   ...
   */
  YOLO12Detector (const std::string &modelPath, const std::string &labelsPath,
                  bool useGPU = false);
  ...
  {
    utils::drawBoundingBoxMask (image, detections, classNames, classColors,
                                maskAlpha);
  }
...
private:
  ...
  std::vector<std::string>
      classNames; // Vector of class names loaded from file
...
cv::Mat
YOLO12Detector::preprocess (const cv::Mat &image, float *&blob,
                            std::vector<int64_t> &inputTensorShape)
{
                             ...
                             float confThreshold, float iouThreshold)
{
  ...
  const float *rawOutput
      = outputTensors[0]
            .GetTensorData<float> (); // Extract raw output data from
      ...
      if (maxScore > confThreshold)
        {
          ...
          nmsBox.x
              += classId * 7680; // Arbitrary offset to differentiate classes
...
crop_detections (const cv::Mat &image,
                 const std::vector<Detection> &detections, bool debug)
{
...
std::vector<Detection>
YOLO12Detector::detect (const cv::Mat &image, float confThreshold,
                        float iouThreshold)
{
