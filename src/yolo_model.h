...
// ===================================
//
// This header defines the YOLO12Detector class for performing object detection
// using the YOLOv12 model. It includes necessary libraries, utility
// structures, and helper functions to facilitate model inference and result
// postprocessing.
//
// Authors:
 ...
 * @brief Header file for the YOLO12Detector class, responsible for object
 * detection using the YOLOv12 model with optimized performance for minimal
 * latency.
 */
 ...
 *
 * @tparam T The type of the value to clamp. Should be an arithmetic type (int,
 * float, etc.).
 ...
 *
 * @note If low > high, the function swaps the bounds automatically to ensure
 * valid behavior.
 */
template<typename T>
...
getClassNames(const std::string& path)
{
...
vectorProduct(const std::vector<int64_t>& vector)
{
  return std::accumulate(
    vector.begin(), vector.end(), 1ull, std::multiplies<size_t>());
}
          ...
          int stride = 32)
{
  ...
  if (auto_) {
    ...
    // Calculate separate padding for left/right and top/bottom to handle odd
    // padding
    int padLeft = dw / 2;
  ...
  if (image.cols != newUnpadW || image.rows != newUnpadH) {
    cv::resize(
      image, outImage, cv::Size(newUnpadW, newUnpadH), 0, 0, cv::INTER_LINEAR);
  } else {
    // Avoid unnecessary copying if dimensions are the same
    outImage = image;
  }
...
scaleCoords(const cv::Size& imageShape,
            BoundingBox coords,
            ...
            bool p_Clip)
{
  ...
  int padX = static_cast<int>(
    std::round((imageShape.width - imageOriginalShape.width * gain) / 2.0f));
  int padY = static_cast<int>(
    std::round((imageShape.height - imageOriginalShape.height * gain) / 2.0f));
  ...
  if (p_Clip) {
    ...
    result.width =
      utils::clamp(result.width, 0, imageOriginalShape.width - result.x);
    result.height =
      utils::clamp(result.height, 0, imageOriginalShape.height - result.y);
  }
  return result;
}
 ...
 * @param scores Vector of confidence scores corresponding to each bounding
 * box.
...
static inline void
NMSBoxes(const std::vector<BoundingBox>& boundingBoxes,
         ...
         std::vector<int>& indices)
{
  ...
  std::sort(
    sortedIndices.begin(), sortedIndices.end(), [&scores](int idx1, int idx2) {
  ...
  for (size_t i = 0; i < sortedIndices.size(); ++i) {
    ...
    for (size_t j = i + 1; j < sortedIndices.size(); ++j) {
      ...
      const float x2 =
        std::min(x2_max, static_cast<float>(compareBox.x + compareBox.width));
      const float y2 =
        std::min(y2_max, static_cast<float>(compareBox.y + compareBox.height));
  ...
  DEBUG_PRINT("NMS completed with " + std::to_string(indices.size()) +
              " indices remaining");
}
...
generateColors(const std::vector<std::string>& classNames, int seed = 42)
{
  ...
  // Compute a hash key based on class names to identify unique class
  // configurations
  size_t hashKey = 0;
  for (const auto& name : classNames) {
    hashKey ^= std::hash<std::string>{}(name) + 0x9e3779b9 + (hashKey << 6) +
               (hashKey >> 2);
  }
  ...
  std::uniform_int_distribution<int> uni(
    0, 255); // Define distribution for color values

  for (size_t i = 0; i < classNames.size(); ++i) {
    colors.emplace_back(
      cv::Scalar(uni(rng), uni(rng), uni(rng))); // Generate random BGR color
  }
                ...
                const std::vector<cv::Scalar>& colors)
{
  ...
  for (const auto& detection : detections) {
    ...
    cv::putText(image,
                label,
                cv::Point(detection.box.x + 2, labelY - 2),
                fontFace,
                fontScale,
...
/**
 * @brief Draws bounding boxes and semi-transparent masks on the image based on
 * detections.
 *
 * @param image Image on which to draw.
 * @param detections Vector of detections.
 ...
 */
static inline void
drawBoundingBoxMask(cv::Mat& image,
                    const std::vector<Detection>& detections,
                    const std::vector<std::string>& classNames,
                    ...
                    float maskAlpha = 0.4f)
{
  ...
  // Pre-filter detections to include only those above the confidence threshold
  // and with valid class IDs
  std::vector<const Detection*> filteredDetections;
  for (const auto& detection : detections) {
    if (detection.conf > CONFIDENCE_THRESHOLD && detection.classId >= 0 &&
        static_cast<size_t>(detection.classId) < classNames.size()) {
  ...
  // Blend the maskImage with the original image to apply the semi-transparent
  // masks
  ...
  for (const auto* detection : filteredDetections) {
    ...
    cv::Size labelSize = cv::getTextSize(
      label, cv::FONT_HERSHEY_SIMPLEX, fontSize, textThickness, &baseLine);
...
/**
 * @brief YOLO12Detector class handles loading the YOLO model, preprocessing
...
public:
   ...
   */
  YOLO12Detector(const std::string& modelPath,
   ...
   * @param confThreshold Confidence threshold to filter detections (default is
   * 0.4).
   * @param iouThreshold IoU threshold for Non-Maximum Suppression (default is
   * 0.45).
  ...
  {
    utils::drawBoundingBoxMask(
      image, detections, classNames, classColors, maskAlpha);
  }
...
private:
   ...
   * @param inputTensorShape Reference to vector representing input tensor
   * shape.
...
YOLO12Detector::YOLO12Detector(const std::string& modelPath,
                               const std::string& labelsPath,
                               bool useGPU)
{
  ...
  if (useGPU && cudaAvailable != availableProviders.end()) {
    ...
    if (useGPU) {
      std::cout << "GPU is not supported by your ONNXRuntime build. "
                   "Fallback to CPU."
                << std::endl;
    }
  ...
  isDynamicInputShape =
    ...
    (inputTensorShapeVec[2] == -1 &&
     inputTensorShapeVec[3] == -1); // Check for dynamic dimensions
...
cv::Mat
YOLO12Detector::preprocess(const cv::Mat& image,
                           float*& blob,
                           std::vector<int64_t>& inputTensorShape)
{
  ...
  blob =
    new float[resizedImage.cols * resizedImage.rows * resizedImage.channels()];

  // Split the image into separate channels and store in the blob
  std::vector<cv::Mat> chw(resizedImage.channels());
  for (int i = 0; i < resizedImage.channels(); ++i) {
    chw[i] = cv::Mat(resizedImage.rows,
                     resizedImage.cols,
                     CV_32FC1,
...
YOLO12Detector::postprocess(const cv::Size& originalImageSize,
                            const cv::Size& resizedImageShape,
                            const std::vector<Ort::Value>& outputTensors,
                            float confThreshold,
                            float iouThreshold)
  ...
  for (size_t d = 0; d < num_detections; ++d) {
    ...
    if (maxScore > confThreshold) {
      ...
      BoundingBox scaledBox =
        utils::scaleCoords(resizedImageShape,
...
crop_detections(const cv::Mat& image,
                const std::vector<Detection>& detections,
                bool debug)
{
  ...
  for (const auto& detection : detections) {
    ...
    int height = detection.box.y + detection.box.height;
...
std::vector<Detection>
YOLO12Detector::detect(const cv::Mat& image,
                       float confThreshold,
                       float iouThreshold)
{
  ...
  // Run the inference session with the input tensor and retrieve output
  // tensors
  ...
  std::vector<Detection> detections = postprocess(image.size(),
                                                  resizedImageShape,
                                                  outputTensors,
                                                  confThreshold,
                                                  iouThreshold);
