...
static inline void scale_wh (int original_width,
int original_height,
int* new_width,
int* new_height,
float* ratio,
const int target_size = 640) {
    float r     = std::min (static_cast<float> (target_size) / original_width,
    ...
    for (int c = 0; c < 3; c++) {
        for (int h = 0; h < input.rows; h++) {
            for (int w = 0; w < input.cols; w++) {
                tensor_data[c * input.rows * input.cols + h * input.cols + w] =
...
static inline std::vector<std::string> get_class_names (const std::string& path) {
    ...
    std::cout << "Loaded " << classNames.size () << " class names from " + path
              << std::endl;
    return classNames;
}
...
static inline cv::Mat letterbox (cv::Mat* input_image, int target_size = 640) {
    ...
    cv::Mat canvas =
    cv::Mat (target_size, target_size, input_image->type (), cv::Scalar (0, 0, 0));
