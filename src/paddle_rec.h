...
static inline void permute(const cv::Mat &im, float * data) noexcept {
  int rh = im.rows;
  int rw = im.cols;
  int rc = im.channels();
...
static inline void rec_resize_input(cv::Mat &img , cv::Mat &resize_img, float wh_ratio, bool use_tensorrt, const std::vector<int> &rec_image_shape) noexcept {
  int imgC, imgH, imgW;
...
static inline void rec_normalize_input(cv::Mat &im, const std::vector<float> &mean, const std::vector<float> &scale, const bool is_scale)  {
  double e = 1.0;
  if (is_scale) {
    e /= 255.0;
  }
    ...
    const int rec_batch_num, const int rec_img_h, const int rec_img_w)
{
    ...
    std::shared_ptr<paddle_infer::Predictor> pred = paddle_infer::CreatePredictor(config);
    return pred;
}


static inline void run_rec_model(std::shared_ptr<paddle_infer::Predictor> &char_rec, const std::vector<cv::Mat> &input_img) {
