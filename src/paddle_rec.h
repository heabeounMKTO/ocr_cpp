                 ...
                 const std::vector<int>& rec_image_shape) noexcept
{
  ...
  cv::copyMakeBorder(resize_img,
                     ...
                     0,
                     0,
                     int(imgW - resize_img.cols),
                     cv::BORDER_CONSTANT,
                     { 0, 0, 0 });
}

static inline void
rec_normalize_input(cv::Mat& im,
...
new_paddle_recognizer(const std::string model_dir,
                      const bool use_gpu,
                      const int gpu_id,
                      const int gpu_mem,
                      const int cpu_math_library_num_threads,
...
{
  for (const cv::Mat& part : input_img) {
    int imgH = part.rows;
    int imgW = part.cols;
    std::vector<int> rec_img_shape(imgH, imgW);
    float max_wh = imgW * 1.0 / imgH;
    printf("maxwh %f\n", max_wh);
    cv::Mat resized_img;
    rec_resize_input(part, resized_img, max_wh, false, rec_img_shape);
  }
}
