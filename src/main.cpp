...
int main(int argc, char **argv) {
  ...
  cv::Mat image = cv::imread("/media/hbdesk/hb_desk_ext/kh_id_keyinfo/kh_tg_id/"
                             "b9ea8f60-2c5c-4cc1-a193-333874e78deb.jpeg");
  if (image.empty()) {
  ...
  for (int i = 0; i < _a.size(); i++) {
    ...
    oss << "./test_1" << keyinfo_model->class_names[_a[i].class_id] << ".png";
    fprintf(stdout, "conf %f\n", _a[i].confidence);
    std::cout << "class_name " << keyinfo_model->class_names[_a[i].class_id]
              << std::endl;
    std::cout << "[DEBUG] BOX: " << _a[i].box << std::endl;
    cv::Mat _crop_img = image(_a[i].box);
    std::string fname = oss.str();
    cv::imwrite(fname, _crop_img);
  }
}
